...
1. Der Browser legt nur einen service-role-geschützten `checkout_intent` an. Ein zufälliges HttpOnly-Cookie bindet ihn an denselben Browser. Bei einer neuen Adresse wird das gewählte Passwort serverseitig sofort als bcrypt-Hash abgelegt; das Klartextpasswort wird nie gespeichert. Bei einer bereits vorhandenen Adresse ist eine passende angemeldete Supabase-Sitzung Pflicht. Dabei entstehen weder neuer Auth-User noch Order oder Enrollment.
...
5. Ein signaturgeprüfter Webhook lädt Session, Payment Intent und Customer erneut bei Stripe. Er validiert `payment_status=paid`, Status, Price, Menge, Betrag, Währung, Customer, die vor Stripe autorisierte Identität, finale Rechnungsanschrift und den unveränderlichen Fingerprint.
6. Erst nachdem diese Paid-Evidenz atomar gespeichert ist, bleibt ein vorab authentifiziertes Konto gebunden oder ein neues Auth-Konto wird mit dem gespeicherten Passwort-Hash erstellt. Ein fremdes, zwischenzeitlich unter derselben E-Mail entstandenes Konto wird nie übernommen. Dieselbe Transaktion erzeugt die bezahlte Order, den Consent-Nachweis und genau ein aktives Enrollment; anschließend wird der temporäre Passwort-Hash gelöscht.
7. Für ein durch diesen Checkout neu erstelltes Konto erzeugt ein zweiphasiger Browser-Handshake anschließend sichere Supabase-Cookies und quittiert sie in einem zweiten cookie-gebundenen Aufruf. Bei Bestandskonten bleibt die vor Stripe geprüfte Sitzung maßgeblich. Erst dann wird der Bootstrap einmalig verbraucht und zum Dashboard weitergeleitet.
